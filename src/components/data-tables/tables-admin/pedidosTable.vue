<template>
  <div>
    <va-card :title="$t('Pedidos')">
      <div class="row align--center">
        <div class="flex xs12 md6">
          <va-input
            :value="term"
            :placeholder="$t('Busqueda Por Estado')"
            @input="search"
            removable
          >
        ...
        <template slot="actions" slot-scope="props">
            <div class="flex xs12 xl6">
                <va-button-group>
                    ...
                    <va-button v-if="props.rowData.estados.name == 'Proceso'" title="Aceptar o Rechazar Pedido" v-on:click="changeStatus(props.rowData.id)" outline small  color="success" icon="fa fa-check-circle" />
                </va-button-group>
            </div>
        </template>
      </va-data-table>
...
export default {
  ...
  methods: {
    ...
    async loadTable() {
      ...
      try {
        const result = await axios.get(`${URL}/pedidos`,config);
